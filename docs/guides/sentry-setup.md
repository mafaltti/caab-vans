...
1. Go to **Settings > Auth Tokens** (direct URL: `https://sentry.io/settings/auth-tokens/`)
   - Alternatively: **Settings > Developer Settings > Internal Integrations** — create an integration with **Release** (Admin) and **Organization** (Read) permissions, then copy its token
2. Save the token — you'll need it for source map uploads
...
EAS Build automatically picks this up if set in your EAS environment:

```bash
npx eas env:create --name SENTRY_AUTH_TOKEN --value "your-auth-token" --environment production --visibility secret
```
