...
- Rate limit is 40 req/min per van. Client should handle `429` by backing off.
...
- **Server-side rate limit:** ~2,400 requests/hour per van (40/min). Client should handle `429` gracefully by backing off.
