...
- **FR-008**: When the persisted pointer is missing, stale, or invalid, the resolver MUST fall back to legacy time-based recomputation. A pointer is considered stale when `progress_updated_at` is older than 30 minutes relative to the current request time.
